...
The official code is **cloned, not vendored**: `baselines/rma2/third_party/` is
git-ignored, so this
...
| local path | `baselines/rma2/third_party/rma4rma` (git-ignored) |
...
$PY -m pip install baselines/rma2/third_party/rma4rma/ManiSkill2
$PY -m pip install baselines/rma2/third_party/rma4rma/stable-baselines3
$PY -m pip install -e baselines/rma2/third_party/rma4rma
```

Committed, with the patches applied in the right order, as
`baselines/rma2/patches/rma4rma/install_rma2.sh`.
...
in `baselines/rma2/patches/rma4rma/`, which is committed even though `third_party/` is
not, so the
